...
function Music($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      ...
      $$renderer2.push(`<li${attr_class("artist-card fade-in svelte-1veghs3", void 0, { "visible": visible })}${attr_style(`transition-delay: ${stringify(0.05 * i)}s`)}><img${attr("src", artistImages[artist.slug] ?? `/music/${artist.slug}.svg`)}${attr("alt", artist.name)} class="artist-img svelte-1veghs3" loading="lazy"/> <div class="artist-gradient svelte-1veghs3"${attr_style(`background: linear-gradient(145deg, ${stringify(artist.color1)}, ${stringify(artist.color2)});`)} aria-hidden="true"></div> <div class="artist-overlay svelte-1veghs3"><span class="artist-name svelte-1veghs3">${escape_html(artist.name)}</span></div></li>`);
    }
